...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num)
{
  ...
  if (ini_flag==0)
  {
    ...
    /* Load Matrix M*/
    sprintf(FileName, "cec-2017/M_%d_D%d.txt", func_num,nx);
    ...
    /* Load shift_data */
    sprintf(FileName, "cec-2017/shift_data_%d.txt", func_num);
    fpt = fopen(FileName,"r");
    if (fpt==NULL)
    {
      printf("\n Error: Cannot open input file for reading \n");
    ...
    {
      sprintf(FileName, "cec-2017/shuffle_data_%d_D%d.txt", func_num, nx);
      fpt = fopen(FileName,"r");
      if (fpt==NULL)
      {
        printf("\n Error: Cannot open input file for reading \n");
...
void schaffer_F7_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Schwefel's 1.2  */
{
  ...
  for (i=0; i<nx-1; i++)	
  {
    ...
    s = pow(z[i] * z[i] + z[i + 1] * z[i + 1], 0.5);
    tmp = sin(50.0 * pow(s, 0.2));
    f[0] += pow(s, 0.5) + pow(s, 0.5) * tmp * tmp;
  }
